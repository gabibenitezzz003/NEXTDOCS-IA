...
public class ConstructorSolicitudGemini {
	...
	public String construirClasificacion(SolicitudClasificacionModel solicitud, double temperatura) {
		if (solicitud.getCandidatos().isEmpty()) {
			throw new ValidacionException("No se puede clasificar sin tipos candidatos");
		}
		ObjectNode raiz = objectMapper.createObjectNode();
		ArrayNode contenidos = raiz.putArray("contents");
		ObjectNode contenido = contenidos.addObject();
		ArrayNode partes = contenido.putArray("parts");
		partes.addObject().put("text", InstruccionClasificacion.construir(solicitud));
		ObjectNode adjunto = partes.addObject().putObject("inline_data");
		adjunto.put("mime_type", solicitud.getTipoMime());
		adjunto.put("data", Base64.getEncoder().encodeToString(solicitud.getContenido()));
		...
		configuracion.put("temperature", temperatura);
		configuracion.put("responseMimeType", "application/json");
		configuracion.set("responseSchema", construirEsquemaClasificacion());
		return raiz.toString();
	}

	private ObjectNode construirEsquemaClasificacion() {
		...
		propiedades.putObject(InstruccionClasificacion.CAMPO_TIPO).put("type", "STRING");
		propiedades.putObject(InstruccionClasificacion.CAMPO_CONFIANZA).put("type", "NUMBER");
		propiedades.putObject(InstruccionClasificacion.CAMPO_MOTIVO).put("type", "STRING");
		propiedades.putObject(InstruccionClasificacion.CAMPO_NOMBRE_SUGERIDO).put("type", "STRING");

		ObjectNode campos = propiedades.putObject(InstruccionClasificacion.CAMPO_CAMPOS_SUGERIDOS);
		...
		propiedadesElemento.putObject("etiqueta").put("type", "STRING");
		propiedadesElemento.putObject("tipoDato").put("type", "STRING");
		propiedadesElemento.putObject("requerido").put("type", "BOOLEAN");
		propiedadesElemento.putObject("ejemplo").put("type", "STRING");
		elemento.putArray("required").add("clave");

		ArrayNode requeridos = esquema.putArray("required");
		requeridos.add(InstruccionClasificacion.CAMPO_TIPO);
		requeridos.add(InstruccionClasificacion.CAMPO_CONFIANZA);
		return esquema;
	}
